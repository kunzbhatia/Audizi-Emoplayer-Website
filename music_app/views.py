...
def emoplayer(request):
    ...
    while True:
        ...
        if not ret:
            ...
            cv2.putText(frame,"Press q to take snapshot", (50,50), cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,0),1,cv2.LINE_AA)
    ...
    if(user.is_authenticated):
        if request.method=="POST":
            ...
            playlist = Playlist.objects.filter(playlist_name=emotion).first()
            ...
            currPlaylist = Playlist.objects.filter(playlist_name=emotion).first()
